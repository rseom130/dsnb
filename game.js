...
const specialItemSize = 20; // 스페셜 아이템 크기
...
const playerAcceleratorWaitTime = 6; // (/60) 방향 전환 후 0.1초 뒤에 부스터 작동
...
let playerAcceleratorWaitTimer = 0; // 방향 전환 후 부스터 작동 대기

let touchEventFlag = false; // 멀티터치 방지를 위해
// define variable end
...
// specialItemImg.src = './images/specialItem.png';
class SpecialItem {
    constructor() {
    ...
    draw() {
        ctx.fillStyle = 'blue';
        ctx.fillRect(this.x, this.y, this.width, this.height);
        // ctx.drawImage(specialItemImg, this.x, this.y, this.width, this.height);
    }
}

// const tntItemImg = new Image();
// tntItemImg.src = './images/tntItem.png';
class TntItem {
    constructor() {
    ...
    draw() {
        ctx.fillStyle = 'red';
        ctx.fillRect(this.x, this.y, this.width, this.height);
        // ctx.drawImage(tntItemImg, this.x, this.y, this.width, this.height);
    }
}
// define setting end
    ...
    return function getGameAnimation(timestamp) {
        if(gameFlag) {
            ...
            if(timer<=0) {
                ...
                if(playerAcceleratorFlag && fuelTimer>=1 && playerAcceleratorWaitTimer>=playerAcceleratorWaitTime) { // player accelerator flag is false
                    // player speed accelerator
                    fuelTimer--;
                    playerSpeedAddValue = playerAcceleratorSpeed;
                    playerCharacterTimer = 6;
                } else {
                    // fuel charge
                    playerSpeedAddValue = 0;
                    ...
                    playerCharacterTimer = 12;
                    if(playerAcceleratorFlag) {
                      playerAcceleratorWaitTimer++;
                    } else {
                      playerAcceleratorWaitTimer = 0;
                    }
                }
...
const getGameStart = (getGameAnimation) => {
  // test
    ...
    realTimeInterval = setInterval(function() {
      realTime++;
      $('.real').text(getFloatFixed((realTime/10), 1));
    }, 100);
    // test end
    requestAnimationFrame(getGameAnimation);
};
